...
inline void delay_call(int32_t millisecond, std::function<void()> callback)
{
  ...
  // // std::async를 사용해서 비동기 작업 실행 (main thread block 된다 ?? 정말 ???)
  // auto result = std::async(std::launch::async, [millisecond, callback]()
  // {
  //     std::this_thread::sleep_for(std::chrono::milliseconds(millisecond));
  //     callback();
  // });
...
// 다음 함수를 충분히 테스트 해서 개선하자
// void delayCall(int milliseconds, std::function<void()> func) {
//     std::async(std::launch::async, [milliseconds, func]() {
//         std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));
//         func();
//     });
// }
// void delay_call(int milliseconds, std::function<void()> func) {
//     std::thread([milliseconds, func]() {
//         std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));
//         func();
//     }).detach(); // detach()를 호출하여 메인 스레드를 블록하지 않음
// }
...
{
  std::thread(
      [millisecond, callback, args...]()
      {
        std::this_thread::sleep_for(std::chrono::milliseconds(millisecond));
        callback(args...);
      });

  // // async를 사용한 비동기 (main thread block 된다)
  // auto result = std::async(std::launch::async, [millisecond, callback, args...]()
  // {
  //     std::this_thread::sleep_for(std::chrono::milliseconds(millisecond));
  //     callback(args...);
  // });
