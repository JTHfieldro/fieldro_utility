                                  ...
                                  const std::string& target, const std::string& value)
{
  ...
  if(level == "INFO") level_color = GREEN;
  else if(level == "WARN") level_color = YELLOW;
  else if(level == "ERROR") level_color = RED;
  else level_color = RESET;

  std::string action_color;
  if(action == "PUB") action_color = RED;
  else if(action == "SUB") action_color = BLUE;
  else if(action == "SRV") action_color = MAGENTA;
  else if(action == "PROC") action_color = GREEN;
  else action_color = RESET;
  ...
  // 색상이 포함된 버전 (콘솔용)
  ss_color << tag << '\t' << level_color << "[" << std::left << std::setw(5) << level << "]" << RESET << " "
           << "[" << std::left << _component << "]" << RESET << " " << action_color
           << std::left << std::setw(5) << action << RESET << "| " << std::left << std::setw(37) << target;
